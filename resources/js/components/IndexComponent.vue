<template>
  <section class="py-5 px-2 min-vh-50">
    <div class="container px-4 px-lg-5 my-3 border bg-white" style="border-radius: 35px;">
        <div class="row justify-content-center my-4">
          <div class="col-md-4 col-sm-1 mb-2">
            <select v-model="sSelectedCity" aria-label="City" class="form-select bg-light">
              <option v-for="sCity in aCitySelectOptions" :value="sCity">{{ sCity }}</option>
            </select>
          </div>
          <div class="col-md-2 col-sm-2 text-center">
            <button v-if="(bIsLoading === true)" type="button" class="btn btn-secondary">
              Loading...
            </button>
